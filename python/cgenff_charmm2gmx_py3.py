...
def parse_charmm_parameters(prmlines):
	...
	for line in nocomments:
		...
		if section == "BOND":
			...
			other = map(float,s[3:])
			parameters["ANGL"].append([ai,aj,ak]+list(other))
		elif section == "DIHE":
			if line.find('!'):
				line = line[:line.find('!')]
			s = line.split()
		...
		for line in rtplines:
			if line.find('!'):
				...
				if line.startswith("ATOM"):
					...
					self.G.add_node(node_for_adding = self.natoms, attr = atm[self.natoms])
					self.natoms=self.natoms+1

				if line.startswith("BOND") or line.startswith("DOUB"):
					entry = re.split('\s+', (line.lstrip()).rstrip())
...
#-----------------------------------------------------------------------
	def get_nonplanar_dihedrals(self,angl_params):
		...
		for var in self.dihedrals:
			d1=self.G.nodes[var[0]]['attr']['type']			  
			d2=self.G.nodes[var[1]]['attr']['type']
			d3=self.G.nodes[var[2]]['attr']['type']
			d4=self.G.nodes[var[3]]['attr']['type']
			keep=1
	...
	def write_gmx_itp(self,filename,angl_params):
		...
		for atomi in range(0,self.natoms):
			...
			f.write("%6d %10s %6s %6s %6s %6d %10.3f %10.3f   ;\n" % 
			   ( atomi+1,self.G.nodes[atomi]['attr']['type'],
			   self.G.nodes[atomi]['attr']['resid'],self.name,self.G.nodes[atomi]['attr']['name'],atomi+1,
			   self.G.nodes[atomi]['attr']['charge'],self.G.nodes[atomi]['attr']['mass'] ) )
		f.write("\n")
		f.write("[ bonds ]\n")
	...
	def read_mol2_coor_only(self,filename):
		...
		for line in f.readlines():
			...
			if((section=="ATOM") and (not secflag)):
				...
				if (len(entry) > 1):
					atomi = int(entry[0])-1
					self.G.nodes[atomi]['attr']['x'] = float(entry[2])
					self.G.nodes[atomi]['attr']['y'] = float(entry[3])
					self.G.nodes[atomi]['attr']['z'] = float(entry[4])
...
#-----------------------------------------------------------------------
	def write_pdb(self,f):
		for atomi in range(0,self.natoms):
			if(len(self.G.nodes[atomi]['attr']['name']) > 4):
				print("error in atomgroup.write_pdb(): atom name >	characters")
				exit()
			f.write("%-6s%5d %-4s %-4s%5s%12.3f%8.3f%8.3f%6.2f%6.2f\n" %
				("ATOM",atomi+1,self.G.nodes[atomi]['attr']['name'],self.name,self.G.nodes[atomi]['attr']['resid'],self.coord[atomi][0],
				self.coord[atomi][1],self.coord[atomi][2],1.0,self.G.nodes[atomi]['attr']['beta']))
		f.write("END\n")
